...
The corpus is 366M words across all versions -- about 8% of English
Wikipedia; Chassidus and everything it cites is ~1.3GB of embedding vectors. That is far too small to train a model that knows
...
## Layout
    ...
    store/schema.sql      works, segments, texts, links, chunks
    ...
    embed/chunk.py        texts -> retrieval chunks that cite exact segments
    embed/bench.py        embedding throughput on this GPU, on our chunks
    eval/questions.jsonl  retrieval eval set: question -> refs that answer it
...
## Build the corpus
    ...
    python -m ingest.bucket run            # 11,657 versions, ~11 min

## Status

- [x] 5,043,902 citation edges loaded and indexed
- [x] Full Sefaria ingest: 11,657 versions, 6,462 works, 0 failures
- [x] Complex-work node titles normalized to base works
...
Chassidus on Sefaria is 20.4M Hebrew words against 3.5M English -- **5.8:1,
under 18% translated**. *Chanah Ariel*, *Sha'arei Avodah* and others are 0%.
*Etz Chaim*, the central Arizal text, is ~9% translated. There is no Yiddish
Chassidus at all -- none of the Rebbe's Yiddish material is here.
